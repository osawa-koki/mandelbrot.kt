...
object Main {
  ...
  fun main(args: Array<String>) {
    try {
      ...
      val mandelbrots = configParse(config)
  ...
  fun configParse(config: Element): Array<Mandelbrot?> {
    ...
    val mandelbrotList = arrayOfNulls<Mandelbrot>(mandelbrots.length)
    for (i in 0 until mandelbrots.length) {
      ...
      val color_mode = ColorMode.valueOf(mandelbrot.getElementsByTagName("color_mode").item(0).textContent)
